...
#include "../Inc/ModBusSlave.h"
#include "string.h"

uint8_t ModbusSlave::slaveID = 0;

ModbusSlave::ModbusSlave(uint8_t slave_ID,UART_HandleTypeDef *serailPort,
...
uint8_t ModbusSlave::getSlaveID(){
	return slaveID;
}


ModBusResult ModbusSlave::sendModbusException(
		modbusExceptionCode exceptionCode) {

	//| SLAVE_ID | FUNCTION_CODE | Exception code | CRC     |
	//| 1 BYTE   |  1 BYTE       |    1 BYTE      | 2 BYTES |

	uint8_t* receptionframe = getRxBufferReferece();


	txFrame[0] = receptionframe[0];       		// Slave ID
	txFrame[1] = receptionframe[1] | 0x80;  // Adding 1 to the MSB of the function code
...
ModBusResult ModbusSlave::handleReadHoldingRegs(uint16_t*  holdingRegisterValues) {
	...
	if ((numOfregs < 0) | (numOfregs > 125)) {
		sendModbusException(ILLEGAL_DATA_VALUE);
		return MODBUS_ERROR;
	}
	if (endAddr > NUM_OF_HOLDING_REGS) {
		sendModbusException(ILLEGAL_DATA_ADDRESS);
		return MODBUS_ERROR;
	}
	...
	for (int i = 0; i < numOfregs; i++) {
		txFrame[byteCursor++] = (holdingRegisterValues[startingReg] >> 8)
				& 0xff; // Higher byte
		txFrame[byteCursor++] = (holdingRegisterValues[startingReg]) & 0xff; // Lower  byte
		startingReg++;
	}
	sendModBusRequest(byteCursor);

	return MODBUS_OK;

}
ModBusResult ModbusSlave::handleReadInputRegs(uint16_t*  inputRegisterValues) {
...
ModBusResult ModbusSlave::handleReadCoils(uint8_t* coilValues) {
	...
	if ((numCoils < 1) || (numCoils > 2000)) // maximum no. of coils as per the PDF
			{
		sendModbusException(ILLEGAL_DATA_VALUE);  // send an exception
		return MODBUS_ERROR;
	}
	...
	if (endingAddr > 199) // end coil can not be more than 199 as we only have record of 200 (0-199) coils in total
			{
		sendModbusException(ILLEGAL_DATA_ADDRESS);   // send an exception
		return MODBUS_ERROR;
	}
	memset(txFrame, '\0', 256);

	txFrame[0] = slaveID;  							// slave ID
	...
	sendModBusRequest(txFrame[2] + 3);
	return MODBUS_OK;

}
ModBusResult ModbusSlave::handleReadDiscreteInputs(uint8_t* dicreteInputValues) {
...
ModBusResult ModbusSlave::handleWriteSingleHoldingRegister(uint16_t* holdingRegisterValues) {
...
ModBusResult ModbusSlave::handleWriteMulyipleHoldingRegisters(uint16_t* holdingRegisterValues) {
	...
	for (int regCorsor = 0; regCorsor < numRegs; regCorsor++) {
		...
		holdingRegisterValues[staringtAddr++] = (rxFrame[indx++] << 8)
				| rxFrame[indx++];
	...
	sendModBusRequest(6); // send data... CRC will be calculated in the function itself

	return MODBUS_OK;
}

ModBusResult ModbusSlave::handleWriteSingleCoil(uint8_t* coilValues) {
...
ModBusResult ModbusSlave::handleWriteMultipleCoils(uint8_t* coilValues){
	...
	unpackCoilsFromBuffer(byteCount, numCoils,rxFrame+7, coilValues+(uint8_t)staringtAddr );
...
void ModbusSlave::extractFromByte(uint8_t byte, uint8_t bitToExtract,uint8_t* outPutResuls ){
...
/*
void ModbusSlave::unpackCoilsFromBuffer(uint8_t numerOfBytes, uint8_t numberOfCoils, uint8_t* bytesSource, uint8_t* outPutResults) {
    uint8_t remainingBits = numberOfCoils;
    for (uint8_t byteCounter = 0; byteCounter < numerOfBytes; byteCounter++) {
        uint8_t bitsToExtract = remainingBits >= 8 ? 8 : remainingBits;
        uint8_t currentByte = bytesSource[byteCounter];
        extractFromByte(currentByte, bitsToExtract, outPutResults + byteCounter * 8);
        remainingBits = remainingBits - 8;
    }
}
*/

void ModbusSlave::unpackCoilsFromBuffer(uint8_t numerOfBytes,uint8_t numberOfCoils,uint8_t* bytesSource, uint8_t* outPutResuls){
	uint8_t remainingBits = numberOfCoils;
